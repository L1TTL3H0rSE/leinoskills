...
Открой новую сессию и проверь `/vue-nuxt-workflow`, `/go-backend-workflow`, `/project-memory` в меню команд. Claude также может выбирать skill по описанию задачи. Для проверки обнаружения запускать продуктовую задачу или дополнительный LLM-прогон не требуется.
